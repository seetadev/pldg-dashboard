...
function normalizeDataFields(
  ...
  return data.map((entry) => {
    ...
    if (cohortId === "2") {
      ...
      normalized["Program Week"] = entry["Program Week"]?.split("(")[0].trim() || "";
    }

    // Normalize Engagement Participation
    if (typeof entry["Engagement Participation "] === "string") {
      normalized["Engagement Participation "] = 
...
export function validateDataStructure(data: EngagementData[]): {
  ...
  data.forEach((entry, index) => {
    ...
    requiredFields.forEach(field => {
      ...
      } else if (entry[field] === undefined || entry[field] === null || entry[field] === '') {
        errors.push(`Row ${index + 1}: Empty value for required field '${field}'`);
      }
    });

    // Validate Program Week format
    if (entry["Program Week"] && !/^Week\s\d+$/i.test(entry["Program Week"].trim())) {
      errors.push(
