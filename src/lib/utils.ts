    ...
    return {
      ...
      insights: {
        ...
        metrics: {
          ...
          technicalProgress:
            enhancedData.insights?.metrics.technicalProgress || 0,
          collaborationIndex:
            enhancedData.insights?.metrics.collaborationIndex || 0,
        },
      },
    };
  } catch (error) {
...
export function processData(rawData: any): ProcessedData {
  ...
  return {
    ...
    feedbackSentiment: rawData.feedbackSentiment || {
      positive: 0,
      neutral: 0,
      negative: 0,
    },
...
export function calculateEngagementScore(data: ProcessedData): number {
  ...
  const averageContributors =
    data.engagementTrends.reduce((sum, week) => sum + week.total, 0) /
...
function processEngagementLevels(items: EngagementData[]) {
  return _(items)
    ...
    .map((weekItems, week) => ({
      week: week.replace(/\(.*?\)/, "").trim(),
      highEngagement: weekItems.filter((e) =>
        e["Engagement Participation "]?.includes("3 -")
      ).length,
      mediumEngagement: weekItems.filter((e) =>
        e["Engagement Participation "]?.includes("2 -")
      ).length,
      lowEngagement: weekItems.filter((e) =>
        e["Engagement Participation "]?.includes("1 -")
      ).length,
    }))
    .value();
  ...
  return items
    ...
    .map((item) => {
      ...
      return {
        ...
        additionalCalls: sessionStr
          ? sessionStr.split(",").map((s) => s.trim())
          : [],
        weeklyFeedback: Array.isArray(item["PLDG Feedback"])
          ? item["PLDG Feedback"][0]
          : item["PLDG Feedback"] || "",
      };
    });
}
...
export function enhanceTechPartnerData(
  ...
  return baseData.map((partner) => {
    ...
    const result: EnhancedTechPartnerData = {
      ...
      issueTracking: timeSeriesData.flatMap((week) =>
