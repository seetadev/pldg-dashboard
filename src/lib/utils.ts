...
export async function processEngagementData(
  ...
  const baseProcessedData: ProcessedData = {
    ...
    cohortId,
    cohortInfo: COHORT_DATA[cohortId]
  };
...
export function processData(rawData: any, cohortId: CohortId): ProcessedData {
  const cohortInfo = cohortId ? COHORT_DATA[cohortId] : null;
  const processedIssueMetrics = processIssueMetrics(rawData.issueMetrics || []);

  return {
    weeklyChange: rawData.weeklyChange,
    activeContributors: rawData.activeContributors,
    ...
    programHealth: rawData.programHealth,
