...
import { PopoverRoot as Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover';
import type { DateRange as DayPickerDateRange } from 'react-day-picker';
import { DateRangePicker } from '@/components/ui/date-range-picker';
import { Badge } from '@/components/ui/badge';
import { cn } from '@/lib/utils';
import { TimelineFilters, DateRange } from '@/types/dashboard';
import { ToggleGroupContent as ToggleGroup, ToggleGroupItem } from '@/components/ui/toggle-group';
...
export function TimelineFilterBar({
  ...
  const handleDateChange = (range: DayPickerDateRange | undefined) => {
    onFiltersChange({
      ...filters,
      dateRange: range && range.from && range.to ? {
        from: range.from,
        to: range.to
      } : undefined
    });
    ...
    <div className="mb-4 flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-white/50 backdrop-blur-sm rounded-lg p-4 border shadow-sm">
      <div className="flex flex-wrap gap-3">
        <div className="min-w-[300px]">
          <DateRangePicker
            date={filters.dateRange as DayPickerDateRange}
        ...
        {filters.contributors.length > 0 || filters.cohorts.length > 0 || filters.dateRange ? (
          <Button
            variant="ghost"
            size="sm"
          ...
          >
            <X className="h-4 w-4" />
            <span className="ml-2 hidden lg:inline">Clear Filters</span>
          </Button>
        ) : null}
      </div>
    </div>
